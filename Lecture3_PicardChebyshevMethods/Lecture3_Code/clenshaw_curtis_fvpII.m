...
function [T2,P2,T1,P1,Ta,A] = clenshaw_curtis_fvpII(N,M)
...
Uv = zeros(N,N); % [NxN]
for k = 0:N-1
    Uv(1,k+1) = cos(k*acos(1)); % Const of Integration (CoI)
end

% Compute "Position" Constants of Integration (i.e. evaluated T at tau = -1).
Up = zeros(N+1,N+1); % [NxN]
for k = 0:N
    Up(1,k+1) = cos(k*acos(1)); % Const of Integration (CoI)
end
